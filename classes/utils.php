...
use local_placecom_mcp\controller\authorize_controller as oidc_authorize_controller;
use local_placecom_mcp\form\authorize_form;
use local_placecom_mcp\response_type\id_token as oidc_id_token_response;
...
class utils {
    ...
    public static function generate_secret(): string {
        // Get 32 bytes of cryptographically secure random data (works on Windows, Linux, and macOS).
    ...
    public static function get_oauth_server(): Server {
        ...
        require_once($CFG->dirroot . '/local/placecom_mcp/vendor/bshaffer/oauth2-server-php/src/OAuth2/Autoloader.php');
        Autoloader::register();
        require_once($CFG->dirroot . '/local/placecom_mcp/classes/oidc_jwt.php');
        require_once($CFG->dirroot . '/local/placecom_mcp/classes/response_type/id_token.php');
        require_once($CFG->dirroot . '/local/placecom_mcp/classes/controller/authorize_controller.php');

        $storage = new moodle_oauth_storage([]);

        // Set access token lifetime.
        $accesstokenlifetime = get_config('local_placecom_mcp', 'access_token_lifetime');
        ...
        $refreshtokenlifetime = get_config('local_placecom_mcp', 'refresh_token_lifetime');
    ...
    public static function get_issuer(): string {
        ...
        $issuer = trim((string) get_config('local_placecom_mcp', 'issuer'));
        if ($issuer === '') {
            $issuer = $CFG->wwwroot;
        }
    ...
    public static function get_endpoint_url(string $path): string {
        return self::get_issuer() . '/local/placecom_mcp/' . ltrim($path, '/');
    }
    ...
    public static function get_signing_key_record(?string $clientid = null): ?stdClass {
        ...
        if (!empty($clientid)) {
            $record = $DB->get_record('local_placecom_mcp_public_key', ['client_id' => $clientid]);
            if ($record) {
                return $record;
            }
        }

        $record = $DB->get_record('local_placecom_mcp_public_key', ['client_id' => '']);
        return $record ?: null;
    }
    ...
    public static function get_all_signing_key_records(): array {
        ...
        return $DB->get_records('local_placecom_mcp_public_key', null, 'id ASC');
    }
    ...
    public static function get_openid_configuration(): array {
        ...
        $scopes = $DB->get_fieldset('local_placecom_mcp_scope', 'scope');
        if (empty($scopes)) {
    ...
    public static function authorize_user_scope($userid, $clientid, $scope = 'login'): void {
        ...
        $DB->insert_record('local_placecom_mcp_user_auth_scope', $record);
    }
