 ...
 * Keys are stored in the local_placecom_mcp_public_key table.
 *
 * Usage:
...
if ($DB->record_exists('local_placecom_mcp_public_key', $conditions) && !$force) {
...
if ($force && $existing = $DB->get_record('local_placecom_mcp_public_key', $conditions)) {
    $record->id = $existing->id;
    $DB->update_record('local_placecom_mcp_public_key', $record);
    cli_writeln('✓ Keys regenerated successfully (ID: ' . $record->id . ')');
} else {
    $record->id = $DB->insert_record('local_placecom_mcp_public_key', $record);
