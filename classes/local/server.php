...
namespace local_placecom_mcp\local;
...
class server extends webservice_base_server {
    ...
    public function extract_tool_call(): void {
        if (empty($this->mcprequest->params) || empty($this->mcprequest->params['name'])) {
            throw new moodle_exception('err_missing_tool_name', 'local_placecom_mcp');
        }
    ...
    private function enforce_scope(): void {
        ...
        if (!approved_functions::is_approved($this->functionname)) {
            throw new moodle_exception('err_function_not_approved', 'local_placecom_mcp');
        }
        ...
        $scoperow = $DB->get_record('local_placecom_mcp_token_scope', ['token' => $this->token]);
        ...
        if (!$scoperow) {
            ...
            // row here (see the token-issuance flow that populates this table).
    ...
    public function exception_handler($ex): void {
        ...
        if (in_array($errorcode, $authfailurecodes)) {
            $resourcemetadataurl = $CFG->wwwroot . '/local/placecom_mcp/protected_resource.php';
