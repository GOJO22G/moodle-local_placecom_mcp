...
use context_system;
use local_placecom_mcp\event\access_token_created;
...
class moodle_oauth_storage implements
    ...
    public function checkClientCredentials($clientid, $clientsecret = null) {
        ...
        $clientsecretfield = $DB->get_field('local_placecom_mcp_client', 'client_secret', ['client_id' => $clientid]);
    ...
    public function isPublicClient($clientid) {
        ...
        $client = $DB->get_record('local_placecom_mcp_client', ['client_id' => $clientid]);
        if (!$client) {
            return false;
        }
    ...
    public function getClientDetails($clientid) {
        ...
        if ($client = $DB->get_record('local_placecom_mcp_client', ['client_id' => $clientid])) {
            unset($client->id);
            return (array)$client;
        } else {
            return false;
    ...
    public function setClientDetails($clientid, $clientsecret, $redirecturi, $scope = null) {
        ...
        if ($client = $DB->get_record('local_placecom_mcp_client', ['client_id' => $clientid])) {
            ...
            $DB->update_record('local_placecom_mcp_client', $client);
        } else {
            $client = new stdClass();
    ...
    public function getAccessToken($oauthtoken) {
        ...
        if ($token = $DB->get_record('local_placecom_mcp_access_token', ['access_token' => $hashed])) {
            unset($token->id);
    ...
    public function setAccessToken($accesstoken, $clientid, $userid, $expires, $scope = null) {
        ...
        if ($token = $DB->get_record('local_placecom_mcp_access_token', ['access_token' => $hashed])) {
            ...
            $DB->update_record('local_placecom_mcp_access_token', $token);
            ...
            $token->id = $DB->insert_record('local_placecom_mcp_access_token', $token);
    ...
    public function getAuthorizationCode($code) {
        ...
        if ($authcode = $DB->get_record('local_placecom_mcp_authorization_code', ['authorization_code' => $code])) {
            unset($authcode->id);
    ...
    public function setAuthorizationCode(
        ...
        if ($authcode = $DB->get_record('local_placecom_mcp_authorization_code', ['authorization_code' => $code])) {
            ...
            $DB->update_record('local_placecom_mcp_authorization_code', $authcode);
        } else {
            ...
            $DB->insert_record('local_placecom_mcp_authorization_code', $authcode);
        }
    ...
    public function expireAuthorizationCode($code) {
        ...
        $DB->delete_records('local_placecom_mcp_authorization_code', ['authorization_code' => $code]);
    }
    ...
    public function getRefreshToken($refreshtoken) {
        ...
        if ($token = $DB->get_record('local_placecom_mcp_refresh_token', ['refresh_token' => $hashed])) {
            unset($token->id);
    ...
    public function unsetRefreshToken($refreshtoken) {
        ...
        $hashed = self::hash_token($refreshtoken);
        return $DB->delete_records('local_placecom_mcp_refresh_token', ['refresh_token' => $hashed]);
    }
    ...
    public function setRefreshToken($refreshtoken, $clientid, $userid, $expires, $scope = null) {
        ...
        $DB->insert_record('local_placecom_mcp_refresh_token', $token);
    ...
    public function scopeExists($scope) {
        ...
        if ($scope) {
            ...
            $scopecount = $DB->count_records_select('local_placecom_mcp_scope', "scope $scopesql", $params);
    ...
    public function getClientKey($clientid, $subject) {
        ...
        return $DB->get_field('local_placecom_mcp_jwt', 'public_key', ['client_id' => $clientid, 'subject' => $subject]);
    }
    ...
    public function getJti($clientid, $subject, $audience, $expiration, $jti) {
        throw new moodle_exception('not_implemented', 'local_placecom_mcp');
    }
    ...
    public function getPublicKey($clientid = null) {
        ...
        if ($clientid) {
            $key = $DB->get_field('local_placecom_mcp_public_key', 'public_key', ['client_id' => $clientid]);
            if ($key) {
                return $key;
            }
        }
        ...
        return $DB->get_field('local_placecom_mcp_public_key', 'public_key', ['client_id' => '']);
    }
    ...
    public function getPrivateKey($clientid = null) {
        ...
        // First try to get client-specific key, then fall back to default (empty string).
        if ($clientid) {
            $key = $DB->get_field('local_placecom_mcp_public_key', 'private_key', ['client_id' => $clientid]);
            if ($key) {
                return $key;
            }
        }
        ...
        // Fall back to default key (empty client_id).
        return $DB->get_field('local_placecom_mcp_public_key', 'private_key', ['client_id' => '']);
    }
    ...
    public function getEncryptionAlgorithm($clientid = null) {
        ...
        if ($clientid) {
            $alg = $DB->get_field('local_placecom_mcp_public_key', 'encryption_algorithm', ['client_id' => $clientid]);
            if ($alg) {
                return $alg;
            }
        }
        ...
        // Fall back to default key (empty client_id).
        $alg = $DB->get_field('local_placecom_mcp_public_key', 'encryption_algorithm', ['client_id' => '']);
