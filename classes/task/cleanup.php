...
use local_placecom_mcp\event\access_token_deleted;
...
class cleanup extends scheduled_task {
    ...
    public function get_name() {
        return get_string('task_cleanup', 'local_placecom_mcp');
    }
    ...
    public function execute() {
        ...
        if ($expiredauthcodescount > 0) {
            $DB->delete_records_select('local_placecom_mcp_authorization_code', 'expires < :time', ['time' => $time]);
        ...
        foreach ($accesstokenrecordset as $accesstoken) {
            $DB->delete_records('local_placecom_mcp_access_token', ['id' => $accesstoken->id]);
        ...
        if ($expiredrefreshtokenscount > 0) {
            $DB->delete_records_select('local_placecom_mcp_refresh_token', 'expires < :time', ['time' => $time]);
