...
use local_placecom_mcp\form\oauth_client_form;
use local_placecom_mcp\table\oauth_clients_table;
...
admin_externalpage_setup('local_placecom_mcp_manage_oauth_clients');

$action = optional_param('action', '', PARAM_ALPHANUMEXT);

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('manage_oauth_clients', 'local_placecom_mcp'));
...
switch ($action) {
    case 'edit':
        ...
        if (!$clientrecordtoedit = $DB->get_record('local_placecom_mcp_client', ['id' => $id])) {
            echo $OUTPUT->notification(get_string('oauth_client_not_exists', 'local_placecom_mcp'));
            $viewtable = true;
            break;
        }
        $viewtable = false;
    ...
    case 'add':
        ...
        if (($fromform = $form->get_data()) && confirm_sesskey()) {
            ...
            if (!isset($clientrecordtoedit)) {
                ...
                if (!$clientrecord->id = $DB->insert_record('local_placecom_mcp_client', $clientrecord)) {
                    throw new moodle_exception('error_creating_oauth_client', 'local_placecom_mcp');
                }
            } else {
                $clientrecord->id = $clientrecordtoedit->id;
                if (!$DB->update_record('local_placecom_mcp_client', $clientrecord)) {
                    throw new moodle_exception('error_updating_oauth_client', 'local_placecom_mcp');
                }
            }

            if (!empty($rawsecret)) {
                echo $OUTPUT->notification(
                    get_string('oauth_client_secret_shown_once', 'local_placecom_mcp', $rawsecret),
                    'notifywarning'
                );
            }

            echo $OUTPUT->notification(get_string('oauth_client_changes_saved', 'local_placecom_mcp'), 'notifysuccess');
            $viewtable = true;
            break;
        }
    ...
    case 'delete':
        ...
        if (empty($confirm)) {
            if (!$clientrecordtodelete = $DB->get_record('local_placecom_mcp_client', ['id' => $id])) {
                echo $OUTPUT->notification(get_string('oauth_client_not_exists', 'local_placecom_mcp'));
                $viewtable = true;
                break;
            }
            ...
            $viewtable = false;
            echo $OUTPUT->confirm(
                get_string('delete_oauth_client_confirm', 'local_placecom_mcp'),
                new moodle_url('/local/placecom_mcp/manage_oauth_clients.php', ['action' => 'delete', 'id' => $id, 'confirm' => 1]),
                new moodle_url('/local/placecom_mcp/manage_oauth_clients.php')
            );
        } else {
            if (!$DB->delete_records('local_placecom_mcp_client', ['id' => $id])) {
                throw new moodle_exception('error_deleting_oauth_client', 'local_placecom_mcp');
            }
...
if ($viewtable) {
    ...
    echo html_writer::tag(
        'p',
        html_writer::link(
            new moodle_url('/local/placecom_mcp/manage_oauth_clients.php', ['action' => 'add']),
            get_string('oauth_add_client', 'local_placecom_mcp')
        )
    );
